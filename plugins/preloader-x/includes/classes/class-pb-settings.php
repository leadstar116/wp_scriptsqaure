...
if ( ! class_exists( 'PB_Settings' ) ) {
	class PB_Settings {
		...
		function generate_fields( $settings = array(), $post_id = false, $custom_style = true ) {
			...
			foreach ( $settings as $key => $setting_section ) :
				...
				if ( isset( $setting_section['title'] ) ) {
					printf( '<div class="section_title">%s</div>', $setting_section['title'] );
				}
			...
			if ( $custom_style ) {
				printf( '%s', '.wps-field {padding: 10px 0;}.wps-field .wps-field-inline {display: inline-block;vertical-align: top;}.wps-field .wps-field-title {font-size: 14px;width: 120px;min-width: 120px;font-weight: 500;}.wps-field .wps-field-inputs {margin-left: 15px;width: 76%;min-width: 320px;} .wps-field .wps-field-inputs input[type=text], .wps-field .wps-field-inputs textarea, .wps-field .wps-field-inputs input[type=number]{border-radius:4px; padding: 7px 5px; height: inherit;}' );
			}
			printf( '%s', '.section_title{padding: 0;font-size: 16px;margin: 10px 0;}' );
		 ...
		 * @param $styles
		 */
		function field_generator( $option, $styles = true ) {
			...
			if ( isset( $option['disabled'] ) && $option['disabled'] && ! empty( $this->disabled_notice ) ) {
				printf( '<span class="disabled-notice">%s</span>', $this->disabled_notice );
			}
			...
			if ( ! empty( $details ) ) {
				printf( '<p class="description">%s</p>', $details );
			}
		...
		function generate_gallery( $option ) {
			...
			printf( '<div id="media_preview_%s">%s</div>', $id, $html );
            ...
            <script>
                ;(function ($, window, document) {
                    ...
                    $(document).on('ready', function () {
                        $('#media_preview_<?php echo esc_attr( $id ); ?>').sortable({
                            handle: 'img',
                            revert: false,
                            axis: "x",
                        });
                    ...
                    $('#media_upload_<?php echo esc_attr( $id ); ?>').on('click', function () {
                        let send_attachment_bkp = wp.media.editor.send.attachment;
                        wp.media.editor.send.attachment = function (props, attachment) {

                            let html = "<div><span onclick='this.parentElement.remove()' class='dashicons dashicons-trash'></span><img src='" + attachment.url + "' />";
                            html += "<input type='hidden' name='<?php echo esc_attr( $id ); ?>[]' value='" + attachment.id + "'/>";
                            html += "</div>";

                            $('#media_preview_<?php echo esc_attr( $id ); ?>').append(html);
                        };
            ...
            <style>
                #media_preview_<?php echo esc_attr( $id ); ?> > div {
                ...
                #media_preview_<?php echo esc_attr( $id ); ?> > div:hover span {
                    display: block;
                }

                #media_preview_<?php echo esc_attr( $id ); ?> > div > span {
                    display: none;
                    cursor: pointer;
                    background: #ddd;
                    padding: 2px;
                ...
                #media_preview_<?php echo esc_attr( $id ); ?> > div > img {
                    width: 100%;
                    cursor: move;
                }
            </style>
            ...
            <div class="media_preview">
				...
				<?php if ( in_array( $media_ext, array( 'mp3', 'wav' ) ) ) : ?>
                    ...
                    <div id="media_preview_<?php echo esc_attr( $field_id ); ?>"
                         class="dashicons dashicons-format-audio"></div>
		...
		function generate_select( $option ) {
			...
			foreach ( $args as $key => $val ) {
				...
				if ( is_array( $val ) ) {
					ob_start();
					foreach ( $val as $k => $v ) {
						printf( '%s="%s" ', $k, implode( ' ', (array) $v ) );
					}
					printf( '<option %s value="%s" %s>%s</option>', $value == $key ? "selected" : "",
						$key, ob_get_clean(),
						isset( $val['label'] ) ? $val['label'] : ''
					);
				} else {
					printf( '<option %s value="%s">%s</option>', $value == $key ? "selected" : "", $key, $val );
				}
			}

			printf( '<select %s %s name="%s" id="%s">%s</select>', $disabled, $required, $id, $field_id, ob_get_clean() );
		}
            ...
            <div class="image-select">
				...
				foreach ( $args as $key => $val ) {
					...
					if ( is_array( $val ) ) {
						...
						printf( '<label class="%2$s"><input %1$s %2$s type="%6$s" name="%3$s%7$s" value="%4$s"><img %5$s /></label>',
							$disabled, $checked, $option_id, $key, ob_get_clean(), $input_type, $multiple ? '[]' : ''
						);
					} else {
						printf( '<label id="%3$s" class="%2$s"><input %1$s %2$s type="%6$s" name="%3$s%7$s" value="%4$s"><img src="%5$s" /></label>',
							$disabled, $checked, $option_id, $key, $val, $input_type, $multiple ? '[]' : ''
						);
					}
				}
				?>
		...
		function display_function() {
            ...
            <div class="wrap">
				...
				if ( $this->show_submit_button() ) {
					...
					printf( '%s', $this->get_settings_fields_html() );
				}
		...
		function get_posts_array( $string, $option ) {
			...
			if ( ! post_type_exists( $post_type ) ) {
				return new WP_Error( 'not_found', sprintf( esc_html__( 'Post type %s does not exists !', '' ), $post_type ) );
			}
